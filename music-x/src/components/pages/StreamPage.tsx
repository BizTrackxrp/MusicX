        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {featuredAlbums.map((album) => (
            <div
              ...
              className="group relative bg-zinc-900/50 rounded-2xl p-4 hover:bg-zinc-800/50 transition-all cursor-pointer border border-zinc-800 hover:border-zinc-700"
            >
              <div className="relative mb-4">
                ...
                <button
                  ...
                  className="absolute bottom-3 right-3 w-14 h-14 bg-emerald-500 rounded-full flex items-center justify-center shadow-lg shadow-emerald-500/30 opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all hover:scale-105"
                >
              ...
              <div className="flex items-center justify-between mt-3">
                <span className="text-emerald-400 font-medium">{album.albumPrice} XRP</span>
                <span className="text-zinc-500 text-sm">
                  {album.completeAlbumsAvailable}/{album.totalMinted} albums
                </span>
              </div>
            </div>
          ))}
      ...
      <section>
        ...
        <div className="bg-zinc-900/30 rounded-2xl border border-zinc-800 overflow-hidden">
          {trendingTracks.map((track, index) => (
            <div
              key={track.id}
              className={`flex items-center gap-4 p-4 hover:bg-zinc-800/50 transition-colors cursor-pointer group ${
                index !== trendingTracks.length - 1 ? 'border-b border-zinc-800/50' : ''
              }`}
