 ...
 * can purchase without Xaman. UI is wallet-aware.
 */
...
const PurchasePage = {
  ...
  walletLogo() {
    return this.isBifrost()
      ? '/public/bifrost-logo.png'
      : '/public/Xaman-logo.png';
  },
  ...
  async init() {
    ...
    if (!releaseId) { this.showError('No release specified'); return; }
    if (!AppState.user?.address) { this.showLoginRequired(); return; }

    try {
      const data = await API.getRelease(releaseId);
      this.release = data?.release;
      if (!this.release) { this.showError('Release not found'); return; }

      if (trackId && !this.isAlbum) {
        this.track = this.release.tracks?.find(t => t.id == trackId);
        if (!this.track) { this.showError('Track not found'); return; }
      }
  ...
  render() {
    ...
    content.innerHTML = `
              ...
              <div class="xaman-instructions">
                ...
                <div class="xaman-tip">
                  ...
                  <div>
                    ...
                    ${isBifrost
                      ? `<p>You'll need to approve each track in your Bifrost app. Check for push notifications between each signature.</p>`
      ...
      <style>
        ...
        .xaman-logo { width: 48px; height: 48px; border-radius: 12px; object-fit: contain; }
  ...
  updateAlbumProgress(current, total, trackTitle, phase = 'minting') {
    ...
    if (phase === 'minting') { labelEl.textContent = 'Preparing NFT...'; trackEl.textContent = `"${trackTitle}"`; }
    else if (phase === 'ready') { labelEl.textContent = 'Ready to accept'; trackEl.textContent = `"${trackTitle}" - tap below to sign`; }
    else if (phase === 'accepting') { labelEl.textContent = 'Accepting NFT...'; trackEl.textContent = `"${trackTitle}"`; }
    else if (phase === 'verifying') { labelEl.textContent = 'Verifying transfer...'; trackEl.textContent = `"${trackTitle}" - checking on-chain`; }
    else if (phase === 'complete') { labelEl.textContent = 'NFT received!'; trackEl.innerHTML = `<span style="color: var(--success)">✓</span> "${trackTitle}"`; }
  },
  ...
  showAcceptNFTStep(updateStatus, current, total, trackTitle = null) {
    return new Promise((resolve) => {
      ...
      const btnLabel = isBifrost ? `Sign in Bifrost` : `Tap to Open ${walletName}`;
      const subText = isBifrost
        ? 'Check your Bifrost app for a signing notification'
        : `Tap the button above to sign in ${walletName}`;
  ...
  async startPurchase() {
    ...
    const updateStatus = (text, sub, type = 'loading') => {
      ...
      if (type === 'loading') {
        ...
        iconEl.innerHTML = `<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>`;
  ...
  async processSinglePurchase(updateStatus) {
    ...
    // ← ActiveWallet instead of XamanWallet
  ...
  async processAlbumPurchase(updateStatus) {
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      updateStatus(`Preparing NFT ${i + 1}/${trackCount}`, `"${track.title}" - minting...`);
      ...
      await this.showAcceptNFTStep(updateStatus, i + 1, trackCount, track.title);

      this.updateAlbumProgress(i, trackCount, track.title, 'accepting');
      updateStatus(`Accepting NFT ${i + 1}/${trackCount}`, `"${track.title}" - sign in ${walletName}`);
      ...
      updateStatus(`Verifying NFT ${i + 1}/${trackCount}`, `"${track.title}" - confirming on-chain...`);
      ...
      if (!verified) {
        ...
        failedTracks.push(track.title);
        updateStatus(`Transfer Issue — Track ${i + 1}`, `"${track.title}" may not have transferred. Continuing...`, 'error');
        await new Promise(r => setTimeout(r, 2000));
        continue;
      }
  ...
  showError(message) {
    ...
    content.innerHTML = `
      <div class="purchase-page">
        <div class="purchase-container">
          <div class="purchase-error" style="text-align:center;padding:64px;">
            <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="var(--error)" stroke-width="2">
              <circle cx="12" cy="12" r="10"></circle>
              <line x1="15" y1="9" x2="9" y2="15"></line>
              <line x1="9" y1="9" x2="15" y2="15"></line>
            </svg>
            <h2 style="font-size:24px;margin:24px 0 12px;color:var(--text-primary);">Error</h2>
      ...
      <div class="purchase-page">
        <div class="purchase-container">
          <div class="purchase-error" style="text-align:center;padding:64px;">
            ...
            <h2 style="font-size:24px;margin:24px 0 12px;color:var(--text-primary);">Sold Out</h2>
      ...
      <div class="purchase-page">
        <div class="purchase-container">
          <div class="purchase-error" style="text-align:center;padding:64px;">
            ...
            <h2 style="font-size:24px;margin:24px 0 12px;color:var(--text-primary);">Connect Wallet</h2>
            <p style="color:var(--text-muted);">Connect your wallet to make purchases.</p>
